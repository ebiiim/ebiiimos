...
void CalcLoadAddressRange(Elf64_Ehdr* ehdr, UINT64* first, UINT64* last) {
    Elf64_Phdr* phdr = (Elf64_Phdr*)((UINT64)ehdr + ehdr->e_phoff); // program headers
    *first = MAX_UINT64;
    *last = 0;
...
EFI_STATUS EFIAPI UefiMain( EFI_HANDLE image_handle, EFI_SYSTEM_TABLE* system_table) {
    ...
    Print(L"Resolution: %ux%u, Pixel Format: %s, %u px/line\n",
    ...
    if (EFI_ERROR(status)) {
        Print(L"failed to load kernel file to buffer: %r\n", status);
        Halt();
    }
    ...
    // call kernel
    UINT64 entry_addr = *(UINT64*)(kernel_first_addr + 24); // ELF has 24 bytes offset to entry point address
